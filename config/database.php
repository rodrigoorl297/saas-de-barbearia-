...
/** Faixas de nível do fidelidade (configuráveis em Configurações/Fidelidade). */
...
{
    foreach (store_read('loyalty') as $m) {
        if ((string)($m['client_phone'] ?? '') === $phone && $phone !== '') {
            return $m;
        }
    }
    return null;
 ...
 * Usado pelo ajuste manual do dono, resgate de recompensas e pontuação automática.
 */
function loyalty_add_points(string $phone, string $name, int $delta, string $reason, array $meta = []): array
{
    ...
    $idx = null;
    foreach ($rows as $i => $m) {
        if ((string)($m['client_phone'] ?? '') === $phone) {
            $idx = $i;
            break;
        }
    }
    ...
    // Mantém só os últimos 50 lançamentos por cliente.
